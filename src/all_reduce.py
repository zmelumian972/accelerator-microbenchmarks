...
def all_reduce_sum(matrix_dim):
    ...
    matrix_distributed = jax.device_put_sharded(
        matrix_split, jax.local_devices()
    )
    ...
    matrix_size_gbyte_to_bandwidth[matrix_size_gbyte] = (
        achieved_bandwidth_gbyte_s
    )
    print(
...
def run_benchmark():
    ...
    max_achieved_bandwidth_gbyte_s = max(
        matrix_size_gbyte_to_bandwidth.values()
    )
